...
dayConv : Either Char (Char, Char) -> Integer
dayConv (Left d) = int d
dayConv (Right (t, d)) = 10 * int t + int d

monthConv : Either Char Char -> Integer
monthConv (Left c) = int c
monthConv (Right c) = 10 + int c

||| hour regex
hour : TyRE Integer
hour = hourConv <$> r "([01][0-9])!|(2[0-4]!)"

||| minutes or seconds regex
mOrS : TyRE Integer
mOrS = (\(t, d) => 10 * int t + int d) `map` r "([0-5][0-9])!"

milis : TyRE Integer
milis = (\case (h, t, d) => 100 * h + 10 * t + d) `map` rh "{}{}{}" [digit, digit, digit]
...
||| Date regex patter, with seperator
dateWithSep : TyRE () -> TyRE Date
dateWithSep sep =
    let d30 = (30, ) . monthConv . snd <$> rh "30{}((0?[13456789]!)|(1[012]!))" [sep]
        d31 = (31, ) . monthConv . snd <$> rh "30{}((0?[13578]!)|(1[02]!))" [sep]
        upTo20 = mapSnd snd <$> rh "{}{}{}" [day, sep, month]
        dayMonth = upTo20 `or` d30 `or` d31
    in (\((d, m), _, y) => D y m d) <$> rh "{}{}{}" [dayMonth, sep, year]
...
|||     YYYY-MM-DDTHH:mm(+-)HH:mm (1999-03-24T22:53+01:00)
|||     YYYY-MM-DDTHH:mm:ss (1999-03-24T22:53:34)
|||     YYYY-MM-DDTHH:mm:ss(+-)HH:mm (1999-03-24T22:53:34+01:00)
|||     YYYY-MM-DDTHH:mm:ss.sss (1999-03-24T22:53:34.011)
|||     YYYY-MM-DDTHH:mm:ss.sss(+-)HH:mm (1999-03-24T22:53:34.011+01:00)
export
iso : TyRE DateTime
iso =
    let date = dateWithSep $ r "-"

        -- Seconds optionally with miliseconds
        sWithMs = mapSnd (fromMaybe 0) <$> rh "{}(.{})?" [mOrS, milis]

        timezone = rh "(\\+|-)!{}:{}" [hour, mOrS]

        time = rh "{}:{}(:{})?{}?" [hour, mOrS, sWithMs, timezone]
    in ?conv <$> time
-- iso =
